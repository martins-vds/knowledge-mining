      ...
      sed -i "s~__STORAGE_RESOURCE_ID__~${STORAGE_RESOURCE_ID}~g" $BASE_DATA_SOURCE_FILE > $DATA_SOURCE_FILE
      sed -i "s/__STORAGE_CONTAINER__/${STORAGE_ACCOUNT_CONTAINER}/g" $DATA_SOURCE_FILE > $DATA_SOURCE_FILE
      ...
      sed -i "s/__COG_SERVICES_KEY__/${COGNITIVE_SERVICE_SECRET}/g" $BASE_SKILLS_FILE > $SKILLS_FILE
      ...
      sed -i "s/__DATASOURCE_NAME__/${DATA_SOURCE_NAME}/g" $BASE_INDEXER_FILE > $INDEXER_FILE
      sed -i "s/__INDEX_NAME__/${INDEX_NAME}/g" $INDEXER_FILE > $INDEXER_FILE
      sed -i "s/__SKILLSET_NAME__/${SKILLS_NAME}/g" $INDEXER_FILE > $INDEXER_FILE
