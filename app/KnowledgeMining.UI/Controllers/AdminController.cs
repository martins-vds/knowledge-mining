...
namespace KnowledgeMining.UI.Controllers
{
    public class AdminController : Controller
    {
        ...
        [HttpGet]
        public async Task<IActionResult> Customize(CancellationToken cancellationToken)
        {
            var model = new CustomizeViewModel
            {
                NavBar = await ReadCssColorProperties("navbar", cancellationToken)
            };
            return View(model);
        }

        [HttpPost]
        public async Task<IActionResult> CustomizeNavBar(CancellationToken cancellationToken)
        {
            var cssString = "";
        ...
        private async Task<ColorSettings> ReadCssColorProperties(string section, CancellationToken cancellationToken)
        {
            var colorSettings = new ColorSettings();
            var webPath = _hostingEnvironment.WebRootPath;
            var path = Path.Combine("", webPath + $"\\css\\{section}.css");

            var css = await System.IO.File.ReadAllTextAsync(path, cancellationToken);
        ...
        public async Task<IActionResult> DownloadCustomCss(CancellationToken cancellationToken)
        {
            var webPath = _hostingEnvironment.WebRootPath;
        ...
        [HttpPost]
        public async Task<IActionResult> UploadCustomCss(CancellationToken cancellationToken)
        {
